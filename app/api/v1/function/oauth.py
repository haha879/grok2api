...
OAuth tokens are stored in memory and treated as valid function credentials.
"""
...
def _get_redirect_uri() -> str:
    ...
    return f"{app_url}/v1/function/oauth/linuxdo/callback"
...
async def oauth_config():
    ...
    return {"linuxdo_enabled": _is_linuxdo_enabled() and is_function_enabled()}


@router.get("/linuxdo/login")
async def linuxdo_login():
    """Redirect user to linux.do authorization page."""
    if not _is_linuxdo_enabled() or not is_function_enabled():
