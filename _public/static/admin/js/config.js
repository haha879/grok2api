...
const LOCALE_MAP = {
  ...
  "credits": {
    "label": "积分系统",
    "enabled": { title: "启用积分系统", desc: "是否启用积分系统（仅对 OAuth 登录用户生效，Function Key 用户不受限）。" },
    "initial_credits": { title: "初始积分", desc: "新用户首次登录时赠送的初始积分。" },
    "daily_checkin_credits": { title: "签到积分", desc: "每日签到获得的积分奖励。" },
    "image_cost": { title: "图片消耗", desc: "每次图片生成消耗的积分。" },
    "image_edit_cost": { title: "图片编辑消耗", desc: "每次图片编辑消耗的积分。" },
    "video_cost": { title: "视频消耗", desc: "每次视频生成消耗的积分。" }
  }
};
...
const SECTION_DESCRIPTIONS = {
  ...
  "credits": "积分系统仅对通过 OAuth 登录的用户生效，使用 Function Key 直接访问的用户不受积分限制。"
};
